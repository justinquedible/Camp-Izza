...
export const NewManageCampers: React.FC<ProgramProps> = () => {
                            ...
                            <table className={"manageTable"}>
                                ...
                                {Object.entries(report).map(([k, v], index) =>
                                    <tr>
                                        <td>
                                            <p>Week {index + 1}</p>
                                            {Object.entries(v).filter(([x, y]) => x == "startDate" || x == "endDate").sort((a, b) => b[0].localeCompare(a[0])).map(([key, val]) =>
                                                <div>
                                                    {key == "startDate" ? <p>  {val} → </p> : <p> {val}   </p>}
                                                </div>
                                            )}
                                        </td>
                                        {Object.entries(v).filter(([x, y]) => x != "startDate" && x != "endDate").length == 0 ? <i><br/> No Records
                                            found </i> : Object.entries(v).filter(([x, y]) => x != "startDate" && x != "endDate").map(([key, val]) =>
                                            <td>Date: {key} <br/>Present for: {val} </td>
                                        )}
                                    </tr>
                                )}
    ...
    const Campers = () => <CardDeck style={{display: 'block'}}>
        {
                        ...
                        <Card.Body>
                            ...
                            <div className={"row"}>
                                <div className={"column4"}>
                                    <a className={"blue"} onClick={() => handleAdminClick(item.firstname, item.id)} href={"/#/admin/AdminCamperForm"} >Go to camper info →</a>
                                </div>
                                <div className={"column4"}>
                                    <p>Group: {item.group.substring(0, 1).toUpperCase()}{item.group.substring(1, item.group.length - 1)} {item.group.substring(item.group.length - 1)}</p>
                                </div>
                                <div className={"column4"}>
                                    <a className={"blue"} onClick={handlePopup}>Attendance Report</a>
                                </div>
                                <div className={"column4"}>
                                    <a className={"red"}  onClick={() => handleAdminClick(item.firstname, item.id)} href={"/#/admin/emergencyform"} >Emergency Form</a>
                                </div>
                            </div>
                            ...
                            <input type={"Number"} placeholder={"$0.00"} defaultValue={item.credit} className={"resizeCredit"} onBlur={handleCreditChange('credit')}/>
                            <Button className={"resizeCredit"} type="submit" onClick={() => handleCreditUpdate(item.id)}>Adjust Credit</Button>
    ...
    const handleNewSubmit = async (e: {preventDefault: () => void; }) => {
        ...
        if (queryValues.year == -1) { // Not production ready. Still need to fix this, but for now I needed a number type bc its in the same format as the year indexer.
            ...
            await AdminService.getCampersInYear(queryValues.year).then((response: { data: CamperProps[]; }) => {
    ...
    const handleSubmit = () => {
        ...
        if (queryValues.year == -1) { // Not production ready. Still need to fix this, but for now I needed a number type bc its in the same format as the year indexer.
            ...
            AdminService.getCampersInYear(queryValues.year).then((response: { data: CamperProps[]; }) => {
    ...
    return (
        <div className="ManageCampers">
            ...
            <Container className="Admin-Buttons">
                ...
                <h6> Add, remove, and edit camper information. </h6><br/><br/>
                <form >
                    <p>SUMMER:</p>
                    <select className={"resize"}  defaultValue={queryValues.year} onChange={handleChange('year')}>
                        <option disabled value={0} >-- Select a summer --</option>
                        <option value={new Date().getFullYear()}>Currently
                            Registered: {new Date().getFullYear()}</option>
                    ...
                    <Button variant="success" className={"resize"} type="submit" onClick={handleNewSubmit}> Select </Button>
                </form>
                <br/>
