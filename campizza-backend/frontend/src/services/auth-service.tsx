...
const counselorRegister = (firstName: string, lastName: string, email: string,  password: string) => {
...
const verifyAndUpdatePass = (token: string, password: string) => {
    ...
    return axios.post(API_URL + "confirm-reset", {
        token,
    })
        .then((response) => {
...
const getPendingCounselors = () => {
    ...
    return axios.get(API_URL + "pending-counselors", {
...
const getAllCounselors = () => {
    ...
    return axios.get(API_URL + "all-counselors", {
...
const getArchivedCounselors = () => {
    ...
    return axios.get(API_URL + "archived-counselors", {
...
const changeCounselorActive = (id: number ) => {
    ...
    return axios.post(API_URL + "change-counselor-active", payload)
        .catch((error) => {
            if (error.response.status === 400) {
...
const changeCounselorArchive = (id: number ) => {
    ...
    return axios.post(API_URL + "change-counselor-archived", payload)
        .catch((error) => {
            if (error.response.status === 400) {
                window.alert("Error Role");
                return false;
...
const deleteCounselor = (id: number ) => {
    ...
    return axios.post(API_URL + "delete-counselor", payload)
        .catch((error) => {
            if (error.response.status === 400) {
                window.alert("Error Role");
                return false;
