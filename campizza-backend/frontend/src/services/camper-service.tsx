...
const addCamper = (firstName: string,
                   ...
                   gradeNum: number,
                   shirtEnum: string,
                   doctorName: string, doctorPhone: string, insurance: string,
                   policy_holder: string, illnesses: string, illnesses_names: string, allergies: string, allergy_names:string, medication: string, medication_names: string,
                   activities: string, activity_names: string, medical_treatments: string, medical_treatment_names: string,
                   immunizations: string, tetanus_date: string, comment: string, userID: number) => {


    return axios.post(API_URL + "add" ,{
        firstName, lastName, genderEnum,dobDate, schoolName, gradeNum, shirtEnum, doctorName, doctorPhone, insurance, policy_holder, illnesses, illnesses_names,
        allergies, allergy_names, medication, medication_names, activities, activity_names, medical_treatments, medical_treatment_names,
        immunizations, tetanus_date, comment, userID
...
const addCamperSchedule = (name:string, userId: number, numShirts:number,  currentWeeksRegistered:Array<any>) => {
    const date = new Date();
    let currentYear = date.getFullYear();
    return axios.post(API_URL + "addSchedule", {
        name, userId, currentYear, numShirts, currentWeeksRegistered
    });
};
...
export default {
    ...
    assignGroup,
    assignPaid,
    assignCredit, getPaid, getCredit, getCamperInfo
};
